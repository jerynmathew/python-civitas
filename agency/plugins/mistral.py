...
Install:
    pip install python-agency[mistral]

Usage:
    from agency.plugins.mistral import MistralProvider
...
def _compute_cost(model: str, tokens_in: int, tokens_out: int) -> float | None:
    ...
    if pricing is None:
        for key, val in _PRICING_PER_M_TOKENS.items():
            if model.startswith(key.rstrip("-latest")):
                pricing = val
                break
    if pricing is None:
        return None
...
class MistralProvider:
    ...
    Requires ``pip install python-agency[mistral]``.
    ...
    def __init__(
        ...
        if not _HAS_MISTRAL:
            raise ImportError(
                ...
                "Install it with: pip install python-agency[mistral]"
            )
