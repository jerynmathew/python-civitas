...
Wraps an OpenAI Agents SDK ``Agent`` so it gains Civitas supervision, OTEL
...
from civitas.errors import ErrorAction, MessageRoutingError
        ...
        for item in getattr(result, "new_items", []):
            if hasattr(item, "agent") and hasattr(item, "input"):
                try:
                    ...
                    logger.warning(
                        "[OpenAIAgent] handoff to %r failed — not registered in Civitas",
                        item.agent.name,
                    )
